...
data science workflow: EDA → modeling → evaluation → deployment.
...
*(Planned — end-to-end deployment with FastAPI + Docker + AWS)*

---

## Stack
`Python` `XGBoost` `LightGBM` `pandas` `scikit-learn` `SHAP`  
`FastAPI` `Docker` `AWS` `Snowflake` `dbt` `Databricks`
